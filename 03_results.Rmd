...
On average, female sablefish grow faster and reach a far greater size (Figure \@ref(fig:figure13)a,b) compared to males (Figure \@ref(fig:figure13)c,d).
...
During the `r yr` sablefish surveys, `r countStRS[1,3]` previously tagged fish were recovered and re-released live with a new tag.  The majority (79%) were recovered within 50 kilometers of the release site.  Most of these tagged fish (70%) were recaptured within 5 years at liberty (Table \@ref(tab:table4)).  Three fish were recovered a second time and released a third time (Table \@ref(tab:table5)).
...
```{r SablefishOceanText, echo=FALSE}
     ...
     weather  <-  read.csv(paste(path,'figure19.csv',sep=''),header=T) 

     weather$depth_stratum  <-     trimws(weather$depth_stratum)   # trim whitespace

     shallow.temp <- weather[weather$depth_stratum=="RD1",]
     mid.temp     <- weather[weather$depth_stratum=="RD2",]        
     deep.temp    <- weather[weather$depth_stratum=="RD3",]        
       
     weather.max <-  max(shallow.temp$Avgtemp)
     weather.max.shallow <-shallow.temp[shallow.temp$Avgtemp==weather.max,]
     
     weather.max <-  max(mid.temp$Avgtemp)
     weather.max.mid <-mid.temp[mid.temp$Avgtemp==weather.max,]
     
     weather.max <-  max(deep.temp$Avgtemp)
     weather.max.deep <-deep.temp[deep.temp$Avgtemp==weather.max,]  
     
     weather.min <-  min(shallow.temp$Avgtemp)
     weather.min.shallow <-shallow.temp[shallow.temp$Avgtemp==weather.min,]
     
     weather.min <-  min(mid.temp$Avgtemp)
     weather.min.mid <-mid.temp[mid.temp$Avgtemp==weather.min,]
     
     weather.min <-  min(deep.temp$Avgtemp)
     weather.min.deep <-deep.temp[deep.temp$Avgtemp==weather.min,]
...
SBE 39 recorders have been placed on survey fishing sets since 2006. In the shallow waters, the lowest average temperature was `r weather.min.shallow[1,5]` $^\circ$C  in `r weather.min.shallow[1,1]`; the highest average temperature was `r weather.max.shallow[1,5]` $^\circ$C in `r weather.min.shallow[1,1]`. Moving into the mid-depth waters, from 458-823 meters, the lowest average temperature was `r weather.min.mid[1,5]` $^\circ$C in `r weather.min.mid[1,1]`; the highest average temperature was `r weather.max.mid[1,5]` $^\circ$C in `r weather.max.mid[1,1]`. In the deepest waters, the lowest average temperature of `r weather.min.deep[1,5]` $^\circ$C in `r weather.min.deep[1,1]` and the highest average temperature was `r weather.max.deep[1,5]`$^\circ$C  in `r weather.min.deep[1,1]` (Figure \@ref(fig:figure19)).
