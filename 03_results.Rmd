...
There have been distinct distribution patterns across strata of sub-legal sablefish (<55 cm fork length), following the highly anomalous warm ocean conditions of the “The Blob” [@Bond2015].   
More than half of the sub-legal specimens were captured in the southern strata (S~1~) mid-depth waters (RD~2~) in 2014 and shallow waters (RD~1~) in 2015. The sub-legal specimen count was above 50% in both 2017 and 2018 in the northern strata of S~4~ and S~5~  mid-depth waters (RD~2~). In 2019, the sub-legal specimens dominated in all StRS survey strata (S~1~ to S~5~) mid-depth waters (RD~2~). In `r yr`, the.....   \@ref(fig:figure15)). 

## OTHER FISH SAMPLING

Length, sex, maturity, otoliths and DNA samples were collected for the rougheye/blackspotted rockfish complex.  (Appendix \@ref(app:seventh-appendix)).
...
```{r SablefishOceanText, echo=FALSE}
     ...
     shallmin    <-  min(seabird$avgtemp[seabird$depth=="Shallow"])
     shallyrmin  <- (seabird$Year[seabird$depth=="Shallow" & seabird$avgtemp==shallmin])
     shallband1min   <- (seabird$latitude[seabird$depth=="Shallow" & seabird$Year==shallyrmin & seabird$avgtemp==shallmin]) - 0.5
     shallband2min   <- (seabird$latitude[seabird$depth=="Shallow" & seabird$Year==shallyrmin & seabird$avgtemp==shallmin]) + 0.5
     
     shallmax     <- max(seabird$avgtemp[seabird$depth=="Shallow"])
     shallyrmx    <- (seabird$Year[seabird$depth=="Shallow" & seabird$avgtemp==shallmax])
     shallband1mx <- (seabird$latitude[seabird$depth=="Shallow" & seabird$Year==shallyrmx & seabird$avgtemp==shallmax]) - 0.5
     shallband2mx <- (seabird$latitude[seabird$depth=="Shallow" & seabird$Year==shallyrmx & seabird$avgtemp==shallmax]) + 0.5
     
     midmin        <- min(seabird$avgtemp[seabird$depth  == "Mid"])
     midyrmin      <- (seabird$Year[seabird$depth  == "Mid" & seabird$avgtemp  == midmin])
     midband1min   <- (seabird$latitude[seabird$depth == "Mid" & seabird$Year==midyrmin & seabird$avgtemp  == midmin]) - 0.5
     midband2min   <- (seabird$latitude[seabird$depth == "Mid" & seabird$Year==midyrmin & seabird$avgtemp  == midmin]) + 0.5
     
     midmx       <-  max(seabird$avgtemp[seabird$depth  == "Mid"])
     midyrmx     <- (seabird$Year[seabird$depth  == "Mid" & seabird$avgtemp  == midmx])
     midband1mx  <- (seabird$latitude[seabird$depth == "Mid" & seabird$Year==midyrmx & seabird$avgtemp  == midmx]) - 0.5
     midband2mx  <- (seabird$latitude[seabird$depth == "Mid" & seabird$Year==midyrmx & seabird$avgtemp  == midmx]) + 0.5
     
     deepmin        <-  min(seabird$avgtemp[seabird$depth  == "Deep"])
     deepyr.min      <- (seabird$Year[seabird$depth  == "Deep" & seabird$avgtemp  == deepmin])
     deepband1min   <- (seabird$latitude[seabird$depth == "Deep" & seabird$Year==deepyr.min & seabird$avgtemp  == deepmin]) - 0.5
     deepband2min   <- (seabird$latitude[seabird$depth == "Deep" & seabird$Year==deepyr.min & seabird$avgtemp  == deepmin]) + 0.5  
     
     deepmx      <-  max(seabird$avgtemp[seabird$depth  == "Deep"])
     deepyr.max    <- (seabird$Year[seabird$depth  == "Deep" & seabird$avgtemp  == deepmx])
     deepband1mx <- (seabird$latitude[seabird$depth == "Deep" & seabird$Year==deepyr.max & seabird$avgtemp  == deepmx]) - 0.5
     deepband2mx <- (seabird$latitude[seabird$depth == "Deep" & seabird$Year==deepyr.max & seabird$avgtemp  == deepmx]) + 0.5
     
```


## OCEANOGRAPHIC TEMPERATURES AND DEPTHS
Co-plots of average temperatures and average depths by 1-degree latitude intervals from south-west Vancouver Island to northwest Haida Gwaii can be found in Figure \@ref(fig:figure18).  The `r yr` survey data exhibit a general trend of decreasing temperature with depth. 

SBE 39 recorders have been placed on survey fishing sets since 2006. In the shallow waters, the lowest average temperature was `r round(shallmin,1)` $^\circ$C (`r shallyrmin`) within the `r shallband1min`$^\circ$- `r shallband2min`$^\circ$ latitude band.  The highest average temperature was `r round(shallmax,1)` $^\circ$C (`r shallyrmx`) in the southern `r shallband1mx`$^\circ$ - `r shallband2mx`$^\circ$ latitude band. Moving into the mid-depth waters, from 458-823 meters, the lowest average temperature was `r round(midmin,1)` $^\circ$C (`r midyrmin`) within the `r midband1min`$^\circ$-`r midband2min` $^\circ$ latitude band.  The highest average temperature was `r round(midmx,1)` $^\circ$C (`r midyrmx`) in the southern `r midband1mx`$^\circ$- `r midband2mx`$^\circ$ latitude band.  In the deepest waters, the lowest average temperature of `r round(deepmin,1)` $^\circ$C  (`r deepyr.min`) was found in the `r deepband1min`$^\circ$- `r deepband2min`$^\circ$ latitude band and the highest average temperature was `r round(deepmx,1)`$^\circ$C (`r deepyr.max`) in the southern `r deepband1mx`$^\circ$-`r deepband2mx`$^\circ$ latitude band (Figure \@ref(fig:figure19)).
