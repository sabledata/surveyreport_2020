...
```{r figure6, fig.cap='(ref:figure6)',  out.width = "450px", out.height = "576px", results='asis',echo=FALSE, fig.align = "left"}
...
```{r figure7, fig.cap='(ref:figure7)', out.width = "450px", out.height = "576px",results='asis', fig.align = "left"}
...
```{r figure12, fig.cap='(ref:figure12)', out.width = "450px",out.height = "500px", echo=FALSE, fig.align = "center", warning=FALSE, message=FALSE}
    ...
    img <- paste(basepath,'figures/figure12.png',sep='')   # retrieve png 
           knitr::include_graphics(img)
```
(ref:figure13) Average length and ratios of male and female sablefish by year. Counts by sex are labelled on top of the plotted lines.
...
```
(ref:figure15) The percentage of sub-legal sablefish (<55 cm fork length) sampled by area (S~1~-S~5~) and depth strata (S=shallow, RD~1~, M=mid, RD~2~, D=deep, RD~3~) over time. Sub-legal specimen count above 50% sampled shown in blue.
...
```{r figure16, fig.cap='(ref:figure16)',results='asis', out.width = "480px", out.height = "330px", fig.align="center"}
   ...
   tagPlotdata<- paste(" select  dbo.q_Fish_Tag_Release_Simple.rel_year, ",
                       ...
                       "count(dbo.q_Fish_Tag_Release_Simple.TAG_ID) AS count, ",
                       "dbo.WEB_TAG_REVIEW.Total_released, ",
...
(ref:figure19) Average temperatures as reported from the Sea-bird SBE 39 loggers at 1-degree latitude intervals and three depth intervals. The highest/lowest lowest average temperatures for each depth interval over the time series are represented with smaller/larger circles.
...
```{r figure19, fig.cap='(ref:figure19)', results='asis', out.width = "400px",out.height = "600px", fig.align = "center"}
      ...
      # table SEABIRD_ReportLinePlot created  by Sablefish.dbo.Build_Seabird_tables procedure
       ...
       ctd$low      <-  1  # size of geom_point
       
       # geom_point(size=ctd.2$low)
       ctd[ctd$Year == shallyrmin & ctd$latitude == shallband1min + 0.5 & ctd$depth == "Shallow", "low"] <- 2.5
       ctd[ctd$Year == shallyrmx  & ctd$latitude == shallband1mx  + 0.5 & ctd$depth == "Shallow", "low"] <- 3.5
       ctd[ctd$Year == midyrmin   & ctd$latitude == midband1min + 0.5   & ctd$depth == "Mid", "low"]  <- 2.5
       ctd[ctd$Year == midyrmx    & ctd$latitude == midband1mx  + 0.5   & ctd$depth == "Mid", "low"]  <- 3.5
       ctd[ctd$Year == deepyr.min & ctd$latitude == deepband1min + 0.5  & ctd$depth == "Deep", "low"] <- 2.5
       ctd[ctd$Year == deepyr.max & ctd$latitude == deepband1mx  + 0.5  & ctd$depth == "Deep", "low"] <- 3.5       
       
       ctd.2        <-  arrange(transform(ctd[ctd$Year>2005,],
                                          Year=factor(Year, 
       ...
       ggplot( ctd.2,aes(x=latitude, 
               ...
               geom_errorbar(aes(ymax = avgtemp + stderr, ymin=avgtemp - stderr), width = 0.001)
