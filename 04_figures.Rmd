...
```
(ref:figure8) Length frequency distributions across all depth strata (100-750 fathoms) for male (blue) and female (gray) sablefish over time. The red dashed line indicates the 55 cm size limit.

```{r figure8, fig.cap='(ref:figure8)', out.width = "450px",out.height = "500px",echo=FALSE, fig.align = "center",warning=FALSE, message=FALSE}
     knitr::opts_chunk$set(out.height = "\\textheight",  out.width = "\\textwidth")
     png(paste(basepath,'figures/Figure8.png',sep=""), width = 500, height = 550) # starts writing a png to figure folder
...
```
(ref:figure9) Length frequency distributions in shallow depth strata (100-250 fathoms) for male (blue) and female (gray) sablefish over time. The red dashed line indicates the 55 cm size limit.
...
(ref:figure10)  Length frequency distributions across middle depth strata (250-450 fathoms) for male (blue) and female (gray) sablefish over time. The red dashed line indicates the 55 cm size limit.

```{r figure10, fig.cap='(ref:figure10)', out.width = "450px",out.height = "500px", echo=FALSE, fig.align = "center",warning=FALSE, message=FALSE}
     knitr::opts_chunk$set(out.height = "\\textheight",  out.width = "\\textwidth")
     png(paste(basepath,'figures/Figure10.png',sep=""), width = 500, height = 550) # starts writing a png to folder
    ...
    img <-    paste(basepath,'figures/Figure10.png',sep='')   # retrieve png 
              knitr::include_graphics(img)
```
(ref:figure11) Length frequency distributions across deep depth strata (450-750 fathoms) for male (blue) and female (gray) sablefish over time. The red dashed line indicates the 55 cm size limit.
...
```{r figure11, fig.cap='(ref:figure11)', out.width = "450px",out.height = "500px", echo=FALSE, fig.align = "center", warning=FALSE, message=FALSE}
    ...
    img <- paste(basepath,'figures/Figure11.png',sep='')   # retrieve png 
           knitr::include_graphics(img)
```
(ref:figure12) Average length and ratios of male and female sablefish by year. Counts by sex are labelled on top of the plotted lines, male:female ratio by year labelled across lower portion of plot.

```{r figure12, fig.cap='(ref:figure12)',results='asis', out.width = "450px",out.height = "276px", fig.align="center"}

     png(paste(basepath,'figures/figure12.png',sep=''), width = 700, height = 430) # -- starts writing a png to figure folder
...
```{r figure13, fig.cap='(ref:figure13)',results='asis', out.width = "450px", out.height = "264px", fig.align="center"}

    # Sablefish.dbo.procR_Survey_Sablefish_LenWt_shiny queries table GFBIO_SABLEBIO_VW for survey length data	
    png("C:/github/surveyreport2020/figures/figure13.png", width = 700, height = 412) # -- starts writing a png to figure folder
...
```
(ref:figure14) The percentage of sub-legal sablefish (<55 cm fork length) sampled by area and depth strata since the year 201. Sub-legal specimen count above 50% sampled shown in blue.

```{r figure14, fig.cap='(ref:figure14)',results='asis', out.width = "515px",out.height = "401px", fig.align="center"}

  png(paste(basepath,'figures/figure14.png',sep=''), width = 800, height = 624) 
 ...
 write.table(polar,        file = paste(path,"figure14a.csv",sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",")
 write.table(polarsummary, file = paste(path,"figure14b.csv",sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",")
 
 polar        <-  read.csv(paste(path,'figure14a.csv', sep=''),header=T)        # read from csv
 polarsummary <-  read.csv(paste(path,'figure14b.csv', sep=''),header=T)
 ...
 sublabel    <-   (c("S1RD1","S1RD2","S1RD3",
  ...
  while (!is.null(dev.list()))  dev.off()  
    img <-   paste(basepath,'figures/figure14.png',sep='')   # retrieve png from figures folder
             knitr::include_graphics(img)
```
\clearpage

\clearpage
(ref:figure15) Tag recovery rate one year after release for tagged sablefish released on annual surveys in the past 10 years.

```{r figure15, fig.cap='(ref:figure15)',results='asis', out.width = "480px", out.height = "419px", fig.align="center"}
   png(paste(basepath,'figures/figure15.png',sep=''), width = 600, height = 524)  # starts writing a png to figure folder
   ...
   tagPlotdata  <-  " select  dbo.q_Fish_Tag_Release_Simple.rel_year, ISNULL(dbo.q_Fish_Tag_Recovery_Simple.rec_year, 0) AS rec_year, 
    ...
    write.table(tagPlot, file = paste(path,'figure15.csv',sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",")   
    dat      <-  read.csv(paste(path,'figure15.csv',sep=''),header=T) # read from csv
...
(ref:figure18) Average temperatures as reported from the Sea-bird SBE 39 loggers at 1-degree latitude intervals and three depth intervals: shallow: 100-250 fathoms (183 to 457 meters), mid:  250-450 fathoms (458-823 meters) and deep:  450-750 fathoms (824-1372 meters). 

```{r figure18, fig.cap='(ref:figure18)', results='asis', out.width = "400px",out.height = "666px", fig.align = "center"}
      # table SEABIRD_ReportLinePlot created in the Sablefish.dbo.Build_Seabird_tables procedure
    
       png(paste(basepath,'figures/figure18.png',sep=''), width=600, height=1000) # write png to file
       ctddt  <-  "select * from SEABIRD_ReportLinePlot"
       ctd    <-  GetSQLData(ctddt,"Sablefish")   # read from SQL Server
       write.table(ctd, file = paste(path,"figure17.csv",sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",") 
       ctd    <-  read.csv(paste(path,'figure17.csv',sep=''),header=T)  # read from csv
